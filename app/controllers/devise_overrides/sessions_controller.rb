class DeviseOverrides::SessionsController < DeviseTokenAuth::SessionsController
  ...
  def local_auto_login
    return head :not_found unless local_auto_login_enabled?

    @resource = User.order(:id).first
    return render json: { error: 'No user available for local auto-login' }, status: :not_found unless @resource
  ...
  def local_auto_login_enabled?
    ActiveModel::Type::Boolean.new.cast(ENV.fetch('DISABLE_INITIAL_LOGIN', false))
  end
